...
that. On CPU, `test_standalone_driver_compare_single_multi_rank` is bitwise on `gtfn_cpu` only with
...
rtol=1e-14` derived from nothing. On GPU, `--fmad=false` still leaves a ~1.3e-12/step `vn` diff,
root-caused to the batched `cupy.linalg.solve` in
`rbf_interpolation.py`: the batch extent is the rank-local vertex count, and the 6x6 vertex and cell
systems are ill-conditioned enough that assembly, solve and the axis-1 normalisation reductions are
all batch-dependent. Edge RBF (4x4) is clean, hence the dycore looked innocent.
...
Make CI assert instead of print: drop `ICON4PY_DALLCLOSE_PRINT_INSTEAD_OF_FAIL` so a bitwise
...
- **dace.** The repro block in `ci/base.yml` sets its flags regardless of backend, and dace has its
  own nondeterminism: `test_parallel_interpolation` still carries `atol=2e-13`, and
  `ICON4PY_DETERMINISTIC_RBF_COEFFS` fixes RBF, not dace kernels. Gate the assertion on gtfn.
