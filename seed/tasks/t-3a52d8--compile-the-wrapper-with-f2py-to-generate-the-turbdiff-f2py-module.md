...
**Migration notes.** `depends_on: [t-3e07b2]` is stated by the source ("With the wrapper Fortran code in place…"), not inferred. No effort estimate anywhere → `effort_weeks: null`. Status `done` follows from row 22 being Done and from the cycle-34 pitch's statement that the granule with F2Py Python bindings exists and runs on CPU.
