...
**Migration notes.**
- `priority: 2` (could) rather than the row's `High` → 1, because the source marks this step "If time permits" while the other three are unconditional.
...
- `depends_on: [t-31f6c4]` follows the note's stated ordering (it is the last of the numbered steps and needs the verified module); the note does not spell the edge out in prose, so this is the weakest of the three intra-pitch edges.
