...
def _merge_body(base: str, mine: str, theirs: str) -> tuple[str | None, list[str]]:
    ...
    base_lines, mine_lines, theirs_lines = base.splitlines(True), mine.splitlines(True), theirs.splitlines(True)
    ...
    for span, replacement in ours.items():
        for other_span, other_replacement in yours.items():
            ...
            if (overlaps or touching) and replacement != other_replacement:
                conflicts.append(
                    f"  lines {span[0] + 1}-{span[1]}: stored "
                    f"{''.join(other_replacement).strip()!r} · yours {''.join(replacement).strip()!r}"
                )
    if conflicts:
