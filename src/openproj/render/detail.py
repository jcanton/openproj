...
# The drawings button, beside Slide in the same bar \u2014 jcanton, 2026-08-26: the
# control carries Excalidraw's own mark and not a word, the same rule every
# icon in this bar already keeps.
#
...
_DRAW_BUTTON = Markup(
    '<button type="button" id="drawing" aria-label="Drawings" title="Drawings">'
    '<svg viewBox="0 0 24 24" aria-hidden="true"><g transform="scale(.6)">'
    '<path fill="currentColor" d="{}"/></g></svg></button>'
).format(_DRAWING_MARK)
...
<article {% if not creating %}id="{{ e.id }}" {% endif -%}
  ...
  {#- Drawings, between Slide and the switcher — added here in `.editbar`
      rather than as a seventeenth `FORMATS` entry in `controls.py`. A menu
      that lists what the body already embeds is page chrome, the same
      argument Slide is built on two comments up, and not a formatting mark
      that writes into the document at a selection. Absent while creating for
      the same reason Slide is: nothing is stored yet for a drawing to embed
      into. -#}
  <p class="editbar">{% if not creating %}{{ slidebar }}{% endif %}{%
    if not creating %}{{ drawbutton }}{% endif %}{{ viewbar }}{%
    if not creating %}
...
def render_detail(
    ...
    body = _compiled(_DETAIL).render(
        ...
        # Unconditional — nothing about it varies per record — and safe to pass
        # even where the template withholds it: `{% if not creating %}` and the
        # `{% if may_write %}` around the whole bar are what decide if it is
        # ever written to the page, the same way `viewbar` below is passed
        # whether or not `_editing_possible` says show it.
