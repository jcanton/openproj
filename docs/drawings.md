...
about the product — every fork below was decided in conversation. What is still open
is measurement, and the section that names it says so.
...
It is **not** on the editor page. `detail.html` is already 1,110,377 B with Ace, and
Excalidraw's eager JS is roughly 438 kB gzipped including React 19. The bundle is
fetched on the first press of the drawing button — `connect-src 'self'` allows the
fetch, `script-src 'unsafe-inline'` allows the injection — and mounted in a popup over
the editor. An iframe is not an option: `render/shell.py:74-85` is `default-src 'none'`
with no `frame-src`.
...
## The spike, which comes first

No openproj code is written until a throwaway spike answers the questions the CSP asks.
Build the bundle, mount it under `default-src 'none'`, and measure:
...
2. **Does the PNG export path avoid wasm?** The harfbuzz subsetter (a 1.83 MB chunk) is
   pulled for *SVG* export, to inline subsetted `@font-face` data. PNG rasterises through
   canvas and plausibly never needs it, but that is unverified, and the answer decides
   whether `'wasm-unsafe-eval'` has to be added to a policy that refused it once already
   for Automerge (`static/VENDOR.md`: "weakening the policy to run a merge algorithm is
   the wrong trade"). A blocked wasm instantiation is not necessarily loud, so this needs
   a forced-failure probe rather than an eyeball.
3. **How big is the real bundle?** The measured figure is for a `--splitting --format=esm`
   build, and splitting is not available here because chunk fetches need `script-src
   'self'`. The `en`-only, mermaid-free IIFE number is unknown and is the difference
   between heavy and impossible.
4. **How big is a real drawing?** `console.log(blob.size)` on a realistic diagram —
   thirty-odd shapes with text — and on a pathological one with a pasted screenshot
   inside. The ceiling is picked on that number. `MAX_ASSET_BYTES` is 2 MB
   (`web.py:611`) and nobody has measured a scene-bearing PNG against it.
5. **Does the unconditional `esm.sh` font fallback stay quiet?** `ASSETS_FALLBACK_URL` is
   appended to every FontFace src list with no flag to disable it. Under `font-src data:`
   every candidate is refused; what happens next was not established.
...
7. **Does it mount under `--virtual-time-budget`?** `requestAnimationFrame` fires roughly
   never in this harness (`tests/test_editor.py:3395`, `tests/test_seats.py:772`) and
   `ResizeObserver` is delivered on a rendering frame. Whether Excalidraw can be driven by
   `measured_in` at all, and whether it can be given a forced-synchronous-paint hook the
   way Ace's `renderer.updateFull(true)` is used, is the single biggest testability
   unknown.

Green on 1–3 and the openproj side is a few days. Red on wasm or fonts and the fallback
is to ship the storage and menu half with an export-and-drop handshake against
excalidraw.com — which is a strict prefix of this design, so nothing above is wasted.

## What the harness cannot test, and what to do about it

`tests/browser.py:162-171` writes the page to disk and opens `where.as_uri()`. Every
browser test is a `file://` page with `--dump-dom` as the only channel out. So
`fetch('/drawings/<id>.png')` under `connect-src 'self'` is not exercisable there, which
means the open-an-existing-drawing path has no end-to-end test in the existing harness,
and there is no pixel channel to assert that a canvas drew.

What is testable, and is the coverage this ships with: `put_drawing`'s refusals and its
same-bytes convergence, at the store level; the two routes and their `_write_or_refuse`
wrapping, at the web level; `_EMBED_SRC` against both arms and against the negative cases
(`drawings/notadrawing.png`, `drawings/draw-a1b2c3.svg`) in `tests/test_render.py:3726`;
the export copying both directories; the deck inlining a drawing; and the menu's scan as
a pure function over body text. The round trip through a real origin is a manual check
until the harness grows a served-page mode, and this paragraph is the honest record that
it is one.
...
- **The font licences are unsettled.** The package is MIT and the repo LICENSE is MIT, but
  the shipped woff2 files are third-party (Nunito, Lilita One, Assistant, Cascadia Code,
  Liberation Sans, Comic Shanns) with no per-font LICENSE files in the tree, and the
  generated Excalifont's embedded string reads "Copyright (c) 2024 by Excalidraw. All
  rights reserved." `static/VENDOR.md` requires a named licence per vendored file. This
  needs a real answer before any font is committed.
- **CJK is out.** 12.7 MB of font, under every option. Non-Latin text in a drawing is a
