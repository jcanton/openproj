...
**Ace ships unminified.** `src-noconflict`, not `src-min-noconflict` — "readable by a
person" read literally. He answered before this file had measured what that costs, so both
numbers are recorded here and the choice is one directory name in one URL if he wants it
back: 1,307,387 B against 670,198 B, which is 1.89x the 690 KB this repository refused in
writing twice, and it fails the font-url assertion 24 more times than the minified build
because of how it escapes its `data:` URIs. The byte answer was given as "accept ~988,000 B
on a writer's page"; unminified makes that number ~1,626,000 B. Both facts are his to
overturn or to keep.
