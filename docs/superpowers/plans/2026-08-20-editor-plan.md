...
- **Unminified.** `src-noconflict`, not `src-min-noconflict`. `ace.js` 907,754 B and
  `keybinding-vim.js` 268,756 B — **1,176,510 B** without the markdown mode, 1,307,387 B with
  it. That is 1.89x the number this repository refused, and it fails the font-url assertion
  24 more times than the minified build. Every byte figure below is the minified one and is
  superseded by these.
